function x = dirichletrnd(alpha, N)
% x = DIRICHLETRND(alpha,N) 
% Samples N vectors from a Dirichlet with parameters 
% (alpha(1), ..., alpha(k))
% 
% x = dirichletrnd(alpha, N)
%
% x(i,j) = i'th sample of x_j, where x ~ Dir

% See "Bayesian Data Analysis", Gelman et al., p. 482

if sum(alpha <= 0)
  error('All alpha''s must be greater than 0');
end;
...
scale = 1; % arbitrary
for i=1:k
  x(:,i) = gamrnd(alpha(i), scale, N, 1);
  %x(:,i) = gammarnd(alpha(i), scale, N, 1);
end
%x = mk_stochastic(x);
S = sum(x,2); 
x = x ./ repmat(S, 1, k);
