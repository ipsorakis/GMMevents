function [gmm,FrEn,U] = gmmvar_circ(data,K,options);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   [gmm,FrEn] = gmmvar_circ(data,K,options)
%
...
if (nargin<1) 
  disp('Usage: gmm=gmmvar_circ(data,K,options);');
  return;
end;
...
if (min(min(data)) < 0)
    data = data +pi;
    disp('Normalising to [0:2*pi] range');
end;
...
% initialise
for a=1:A,
  gmm(a)=gmmvarinit_circ(data,K(a),options);
...
for k=1:K,
  dc = data_circ(data,gmm.post(k).Norm_Mu); % re-reference data to mod(2*pi)
  ...
  dist=mdist(dc,qp.Norm_Mu,qp.Wish_iB*qp.Wish_alpha);
...
for k = 1:K,
  ...
  weidata=dc'*gmm.pjgx(:,k); % unnormalised sample mean
  ...
  % wrap Mu to [0:2*pi] range
  %Norm_Mu = mod(Norm_Mu,2*pi);

  %Update posterior Wisharts
  Wish_alpha=0.5*gammasum(k)+pr.Wish_alpha;
  dist=dc-ones(N,1)*Norm_Mu';
