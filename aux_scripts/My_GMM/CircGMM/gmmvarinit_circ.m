function [gmm] = gmmvarinit_circ(data,K,options);
% initialises the gaussian mixture model for Variational GMM algorithm
  ...
  switch options.init
   ...
   case 'kmeans'
    ...
    % using circular stuff
   case 'circ'
%       Mu = linspace(2*pi/K,2*pi,K); % even in interval 0,2*pi
%       Cov = ones(1,K)*(2*pi/K)^2; % std as intermean distance to start with
%       Nm = ones(1,K)*length(data)/K;
      emc = km_circ(data,K);
      Mu = emc.m;
      Cov = emc.C;
      Nm = emc.pk*length(data);
      % sample precision
      alpha=gmm.priors.Wish_alpha*2;
      % sample weights
      kappa=gmm.priors.Dir_alpha;
      ...
      for k=1:K,
          % mean posterior
          gmm.post(k).Norm_Cov=Cov(:,k);
          gmm.post(k).Norm_Prec=inv(Cov(:,k));
          gmm.post(k).Norm_Mu=Mu(:,k);
          % covariance posterior
          gmm.post(k).Wish_alpha=alpha;
          gmm.post(k).Wish_iB= gmm.post(k).Norm_Prec/alpha; %gmm.priors.Wish_B ?
          gmm.post(k).Wish_B=inv(gmm.post(k).Wish_iB);
          % weights posterior
          gmm.post(k).Dir_alpha=Nm(k);
      end;
